...
if data is not None:
    ...
    product_dict = {1: "Camera", 3: "Laptop", 4: "Smartphone", 5: "Headphones", 6: "Smartwatch"}

    # Get unique product IDs available in the dataset
    available_ids = data[column_name].unique()

    # Filter only available product IDs that exist in the dictionary
    filtered_product_dict = {pid: product_dict[pid] for pid in available_ids if pid in product_dict}

    # Create a dropdown with product names instead of numbers
    selected_product_name = st.selectbox("📌 Select Product:", list(filtered_product_dict.values()))

    # Find the corresponding product ID from the selected name
    selected_product_id = next(pid for pid, name in filtered_product_dict.items() if name == selected_product_name)

    # Filter reviews for the selected product
    reviews_input = data.loc[data[column_name] == selected_product_id, ['rating', 'review_text']]
    ...
    if st.button("🔍 Generate Summary"):
        if reviews_input.empty:
            st.warning("⚠️ No reviews found for the selected product.")
        else:
